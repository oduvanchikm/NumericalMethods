...
public class RotationMethodJacobiAndPowerMethod
{
    ...
    private static double PowerMethod(int maxIterations, double tolerance, MyMatrix matrix)
    {
        int n = matrix.getRowSize();
        ...
        while (iteration < maxIterations)
        {
            ...
            Console.WriteLine($"Iteration {iteration}: b_k1 = [{string.Join(", ", b_k1.Select(x => x.ToString("F5")))}]");
            ...
            Console.WriteLine($"Iteration {iteration}: b_k1 normalized = [{string.Join(", ", b_k1.Select(x => x.ToString("F5")))}]");
